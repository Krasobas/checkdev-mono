...
import ru.checkdev.notification.circuitbreaker.CircuitBreaker;
...
public class TgAuthCallWebClient implements TgCall {
    ...
    public Mono<Profile> doGet(String url) {
        ...
        Mono<Profile> defVal = Mono.error(new RuntimeException("All retry attempts failed"));
        return retry.exec(() ->
            cb.exec(() -> WebClient.create(uriProvider.getUri(SERVICE_ID))
                .get()
                .uri(url)
                .retrieve()
    ...
    public Mono<Object> doPost(String url, Profile profile) {
        var retry = new Retry(3, 1000);
        var cb = new CircuitBreaker(2);
        Mono<Object> defVal = Mono.error(new RuntimeException("All retry attempts failed"));
        return retry.exec(() ->
            cb.exec(() -> WebClient.create(uriProvider.getUri(SERVICE_ID))
                .post()
                .uri(url)
